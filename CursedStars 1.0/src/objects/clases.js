...
function createSpaceship(playerIndex,playerClass,bot,x,y){
    //Al ser un nuevo jugador en la partida se debe añadir un elemento a la lista de jugadores
    if(players[playerIndex] == null){
        players[playerIndex] = new Player(playerIndex, playerClass);
    } else {
        if(function(players){
            ...
            //ERROR: No caben más jugadores
        } else{
            //ERROR: Slot de jugador ya ocupado
        }
    }
    ...
    if(!bot) game.camera.follow(spaceship[playerIndex]);
}
